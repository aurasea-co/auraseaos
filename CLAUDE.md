...
- `supabaseAdmin` (service role) only in `/app/superadmin/**`, the LINE approve endpoint, and `lib/menudesk/analysis/run-scan.ts`. Everywhere else, use the RLS user client. (The MenuDesk exception is forced by migration 043, which denies all client writes to `dish_analyses`/`uncosted_dishes`/`ai_usage` so a scanner can't forge a green menu — engine output can only be written by a role that bypasses RLS. Ownership is still proven with the user client first; the service client only writes engine output.)
...
- **MenuDesk** W0–W3 done: scan funnel scaffold, anonymous upload, two-pass engine (`npm run analyze`), and the Thai catalogue (~120 ingredients + 100 dishes in `data/th/`, `npm run seed:menudesk` → migration 044). Believability gate passes — a café menu now ranks 1 red / 6 amber / 2 green at ~$0.006, not the placeholder era's uniform red. **Open item before any restaurant sees output:** verify the cost-dominant ingredients against real invoices; entries tagged `wholesale_estimate` (vs `market_survey_2026_08`) are the unverified ones. W4 blurred result done (server-side redaction — `BlurredScanSummary` structurally cannot carry a dish name; blur is presentation only). **Blocker: migration 043 has NEVER been applied — no MenuDesk table exists in Supabase, so the whole funnel is untested end-to-end.** Apply 043 (+ 044 reference data) and enable Anonymous Sign-Ins before trusting any of it. **Next: W5** phone/LINE unlock, then W6–W7 full result + paywall. Crystal Cafe test branch; Loyverse first adapter; Cloudbeds stubbed (`supportsWriteBack=false`).
