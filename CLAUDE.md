...
- **MenuDesk scan funnel moved out** (Aug 2026) to its own repo at `platform/menudesk/`, per Bible Addendum v2.1 §17. What stayed here is the *subscriber* F&B vertical — `/menudesk` dashboard, `fnb_daily_metrics`, the fnb recommendations engine, POS sync — which is part of this spine and is a different surface from the anonymous scan funnel. Don't rebuild the funnel here.
