...
- **MenuDesk** W0–W2 done (scan funnel scaffold, anonymous upload, two-pass engine + `npm run analyze`). **Next: W3** — replace `data/th/seed.ts` placeholder pricing with real Makro-level ingredient prices and ~100 CommonDish recipes; until then the engine's output is mechanically correct but paints every dish red, so it must not be shown to a restaurant. Then W4–W7 funnel. Crystal Cafe test branch; Loyverse first adapter; Cloudbeds stubbed (`supportsWriteBack=false`).
